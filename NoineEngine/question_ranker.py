...
def ranker_tagger():
    ...
    for al in all_words:
        ...
        tuples =(al["syst"])
        listf =(al["objectives"])
        data_type =(al["release"])
        describe_frequency =(al["roles"])
        operator =(al["iterative"])
        variable =(al["variable"])
        illustrate_frequency = (al["objectives"])
    ...
    for al in all_words:
        ...
        event = (al["formation"])
        sets = (al["feedback"])
        iterator = (al["following"])
        output = (al["estimation"])
        est = (al["estimate"])
        cl = (al["statement"])
        cast =(al["types"])
        application_frequency2 = est + event+sets+iterator+output+cl+cast+construct_freq + apply_freq + modify_freq + build_freq + prioritize_freq + report_freq + sketch_freq + produce_freq
